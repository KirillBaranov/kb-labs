...
1. the prepare workflow builds in topological order, stages immutable npm tarballs and receives the binary manifest generated by `prepare-binary-manifest.mjs` from the same candidate assets;
2. its `Prepare release index` step reads those exact tarballs, package manifests and binary metadata;
