...
describe("observability validators", () => {
  ...
  it("rejects invalid payloads", () => {
    ...
    const describeResult =
      validateServiceObservabilityDescribe(invalidDescribe);
    const healthResult = validateServiceObservabilityHealth(invalidHealth);

    expect(describeResult.ok).toBe(false);
    expect(
      describeResult.ok ? [] : describeResult.issues.map((issue) => issue.path),
    ).toEqual(expect.arrayContaining(["contractVersion", "capabilities.0"]));
    expect(healthResult.ok).toBe(false);
    expect(
      healthResult.ok ? [] : healthResult.issues.map((issue) => issue.path),
    ).toEqual(
