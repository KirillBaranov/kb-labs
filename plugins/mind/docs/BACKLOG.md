...
| HyDE (hypothetical embeddings) | optional | 🟡 ported, default OFF | P3 | Y | `retrieval.hyde` flag + `e2e/mind` `hyde-on` A/B. **Live A/B on the 27-query concept golden (2026-06-03): INCONCLUSIVE** — default/expand-on/hyde-on returned byte-identical metrics (hit@1 0.556, hit@5 0.741, mrr 0.636) because the scenario overlays don't reach mind's effective config (see overlay-path bug below), so all three ran the *same* default config. Not a real ON-vs-OFF signal. Baseline `semanticWinRate=0.000` (no semantic-only wins on the clean corpus → BM25 already nails it; HyDE can only help where the query/code vocabularies diverge — i.e. a degraded/messy corpus). Stays OFF; honest verdict needs the overlay-path fix + a degraded-corpus run. |
| Query expansion (lexical-side) | n/a | 🔵 new, default OFF | P2 | Y | `retrieval.expand` flag + `core/src/retrieval/expand.ts` + `e2e/mind` `expand-on` A/B. Same INCONCLUSIVE A/B as HyDE above (overlay never flipped the flag → expand-on ≡ default). Stays OFF; verdict pending overlay-path fix + degraded corpus. |
| **e2e/mind scenario overlays don't reach mind config (profiles-v2)** | n/a | ❌ bug | **P1** | N | Bench A/B blocker. Scenario overlays write **top-level** `{"mind":{"retrieval":{…}}}`, but the authoritative config lives under `profiles[0].products.mind` (profiles-v2). The overlay merges at a path `effectiveIndexConfig`/getConfig never reads, so `hyde`/`expand`/`rerank` flags are silently ignored — every A/B scenario ran identical default config (proven 2026-06-03). Fix: overlay must target the active profile's `products.mind`, or the effective-config merge must map top-level `mind` → active profile. Until then NO retrieval A/B is trustworthy. |
