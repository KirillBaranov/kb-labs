...
describe('WebhookIdempotencyStore', () => {
  ...
  it('first delivery: returns false (not duplicate)', async () => {
    const isDuplicate = await store.checkAndMark('@kb-labs/plugin', 'alert', 'delivery-001');
    expect(isDuplicate).toBe(false);
  });

  it('second delivery with same key: returns true (duplicate)', async () => {
    await store.checkAndMark('@kb-labs/plugin', 'alert', 'delivery-001');
    const isDuplicate = await store.checkAndMark('@kb-labs/plugin', 'alert', 'delivery-001');
  ...
  it('different delivery keys are independent', async () => {
    await store.checkAndMark('@kb-labs/plugin', 'alert', 'delivery-001');
    const isDuplicate = await store.checkAndMark('@kb-labs/plugin', 'alert', 'delivery-002');
    expect(isDuplicate).toBe(false);
  });

  it('different events are isolated (same key, different event)', async () => {
    await store.checkAndMark('@kb-labs/plugin', 'alert', 'delivery-001');
    const isDuplicate = await store.checkAndMark('@kb-labs/plugin', 'push', 'delivery-001');
    expect(isDuplicate).toBe(false);
  });

  it('different pluginIds are isolated', async () => {
    await store.checkAndMark('@kb-labs/plugin-a', 'alert', 'delivery-001');
    const isDuplicate = await store.checkAndMark('@kb-labs/plugin-b', 'alert', 'delivery-001');
    expect(isDuplicate).toBe(false);
  });
});
