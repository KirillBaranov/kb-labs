...
export default defineCommand<unknown, CLIInput<HistoryFlags>, { exitCode: number }>({
  id: 'quality:history',
  description: 'Show quality snapshot history and trends',

  handler: {
    async execute(ctx: PluginContextV3, input: CLIInput<HistoryFlags>): Promise<{ exitCode: number }> {
      const { flags } = input;
