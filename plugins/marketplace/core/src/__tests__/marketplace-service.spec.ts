...
describe('MarketplaceService', () => {
  ...
  describe('install', () => {
    it('installs a plugin and writes to marketplace.lock', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      const result = await service.install(['@kb-labs/test-plugin']);
    ...
    it('installs multiple plugins in one call', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      const result = await service.install(['@kb-labs/a', '@kb-labs/b']);
    ...
    it('catches afterInstall hook errors as warnings', async () => {
      ...
      const result = await service.install(['@kb-labs/hook-fail']);
  ...
  describe('uninstall', () => {
    it('removes plugin from lock and cache', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.install(['@kb-labs/to-remove']);
      ...
      // Uninstall
      await service.uninstall(['@kb-labs/to-remove']);
    ...
    it('uninstalls multiple plugins', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.install(['@kb-labs/x', '@kb-labs/y']);
      await service.uninstall(['@kb-labs/x', '@kb-labs/y']);
  ...
  describe('link', () => {
    it('links a local plugin directory', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      const result = await service.link(path.relative(tmpDir, dir));
    ...
    it('rejects path outside workspace root (path traversal)', async () => {
      ...
      await expect(service.link('../../etc/passwd')).rejects.toThrow('outside workspace root');
    });
  });
  ...
  describe('unlink', () => {
    it('removes linked plugin from lock', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.link(path.relative(tmpDir, dir));
      await service.unlink('@kb-labs/to-unlink');
  ...
  describe('enable / disable', () => {
    it('enables a disabled plugin', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.install(['@kb-labs/toggle']);

      await service.disable('@kb-labs/toggle');
      let lock = await readMarketplaceLock(tmpDir, new DiagnosticCollector());
      expect(lock!.installed['@kb-labs/toggle']!.enabled).toBe(false);

      await service.enable('@kb-labs/toggle');
      lock = await readMarketplaceLock(tmpDir, new DiagnosticCollector());
    ...
    it('throws when enabling a non-existent package', async () => {
      ...
      await expect(service.enable('@kb-labs/ghost')).rejects.toThrow('not found');
    });
    ...
    it('throws when disabling a non-existent package', async () => {
      ...
      await expect(service.disable('@kb-labs/ghost')).rejects.toThrow('not found');
    });
  });
  ...
  describe('list / getEntry', () => {
    it('lists installed plugins', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.install(['@kb-labs/listed']);
    ...
    it('filters by kind', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.install(['@kb-labs/filter-me']);

      const plugins = await service.list({ kind: 'plugin' });
      expect(plugins).toHaveLength(1);

      const adapters = await service.list({ kind: 'adapter' });
    ...
    it('getEntry returns entry or null', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.install(['@kb-labs/entry-get']);

      const entry = await service.getEntry('@kb-labs/entry-get');
      expect(entry).not.toBeNull();
      expect(entry!.version).toBe('1.0.0');

      const missing = await service.getEntry('@kb-labs/ghost');
  ...
  describe('doctor', () => {
    it('returns ok: true when all packages are healthy', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.install(['@kb-labs/healthy']);
    ...
    it('reports error when package directory is missing', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.install(['@kb-labs/will-vanish']);
    ...
    it('warns about integrity mismatch', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.install(['@kb-labs/tampered']);
    ...
    it('reports info about unsigned packages', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.install(['@kb-labs/unsigned']);
  ...
  describe('sync', () => {
    ...
    it('skips packages already in lock', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.install(['@kb-labs/existing']);

      const result = await service.sync({ include: ['packages/*'] });
    ...
    it('marks new entries as disabled by default', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.sync({ include: ['packages/*'], autoEnable: false });
    ...
    it('auto-enables entries when autoEnable is true', async () => {
      ...
      const service = new MarketplaceService({ root: tmpDir, source });
      await service.sync({ include: ['packages/*'], autoEnable: true });
