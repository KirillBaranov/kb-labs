...
test('prepares a sealed index from every exact package in the staged release flow', () => {
  ...
  assert.equal(index.plugins, null);
  assert.equal(index.adapters, null);
  assert.equal(index.platforms[0].profiles, null);
  assert.equal(index.platforms[0].config, undefined);
  assert.deepEqual(index.platforms[0].packages.map(({ package: packageName }) => packageName), [
