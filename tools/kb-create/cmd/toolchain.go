...
func ensureToolchain(nonInteractive bool, manager string) error {
	...
	if status.NodeVersion != "" {
		if major, parseErr := major(status.NodeVersion); parseErr == nil && major < toolchain.NodeMajor {
	...
	if manager == "pnpm" {
		...
		fmt.Printf("[WARN] Updating pnpm %s → %s\n", status.PnpmVersion, toolchain.PnpmVersion)
