...
// Build-time values are injected by GoReleaser. They stay on the root binary
// so the cutover does not change the public release artifact contract.
var (
	version = "dev"
	commit  = "none"
	date    = "unknown"
...
func main() {
	...
	flag.Parse()
	if *showVersion {
		_, _ = fmt.Fprintf(os.Stdout, "kb-create %s (commit %s, built %s)\n", version, commit, date)
		return
	}
...
// normalizeOperationArgs keeps the public CLI ergonomic without retaining any
// legacy command dispatcher: `kb-create apply …` and `kb-create --operation
// apply …` invoke exactly the same V2 operation.
func normalizeOperationArgs() {
	if len(os.Args) < 2 || strings.HasPrefix(os.Args[1], "-") {
		return
	}
	switch os.Args[1] {
	case "plan", "apply", "update", "uninstall", "rollback", "doctor", "wizard":
		os.Args = append([]string{os.Args[0], "--operation", os.Args[1]}, os.Args[2:]...)
	}
}
