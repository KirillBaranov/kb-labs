...
function makeLogger(records: Array<{ message: string; fields?: Record<string, unknown> }>): ILogger {
...
describe("createSseStream", () => {
  it("owns headers, event framing and one close summary", async () => {
    ...
    const stream = createSseStream(
      { id: "req-1", headers: {}, url: "/events", routeOptions: { url: "/events" } } as never,
      reply as never,
    ...
    expect(reply.raw.write).toHaveBeenCalledWith("event: workflow.event\ndata: {\"id\":42}\n\n");
    expect(cleanup).toHaveBeenCalledOnce();
    expect(records.filter((record) => record.fields?.event === "sse.connection.closed")).toHaveLength(1);
    expect(records.find((record) => record.fields?.event === "sse.connection.closed")?.fields).toMatchObject({
      eventsSent: 1,
