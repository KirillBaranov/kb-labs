...
test('RT-01: WebSocket upgrade succeeds through gateway (/hosts/connect)', async ({ page }) => {
  ...
  await page.evaluate(async (gatewayUrl) => {
    ...
    // Server may close immediately without auth — that's fine, upgrade still worked
  })

  // WS was opened (even if quickly closed with auth error)
...
test('RT-02: SSE endpoint streams events (not buffered by proxy)', async ({ page }) => {
  ...
  test.skip(response === null || response.status() === 404, 'SSE events endpoint not available')
...
test('RT-03: gateway forwards Connection/Upgrade headers (not stripped)', async ({ request }) => {
  ...
  // Must not return 400/502 — gateway should handle the upgrade path
  expect([101, 200, 204, 400, 426]).toContain(res.status())
