...
`pnpm --filter @kb-labs/rest-api start` sets `cwd` to the package directory. The canonical service launcher now resolves the project root before loading `.env`, so service config is independent of that package-local working directory.
