...
Source of truth for the schema: [core/config/src/profiles/types.ts](core/config/src/profiles/types.ts).
...
Files under `core/**` get the full `mind-prod` engine; everything else falls back to the faster `mind-fast`. One profile, two behaviors.
