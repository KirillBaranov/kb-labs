...
`getHandlerPermissions(manifest, host, handlerId)` in [plugin-contracts](infra/kb-labs-plugin/packages/plugin-contracts/src/manifest.ts) is the function that computes this — use it in tests or introspection tools if you want to see what a specific handler actually has access to.
