...
All three live in your plugin's CLI package. The canonical example is [kb-labs-commit-plugin](plugins/commit/entry/src/).

## 1 — The manifest entry

From [kb-labs-commit-plugin's manifest](plugins/commit/entry/src/manifest.ts):
