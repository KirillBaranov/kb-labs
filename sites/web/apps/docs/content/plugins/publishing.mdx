...
The [release-manager plugin](plugins/kb-labs-release-manager/) in the reference monorepo handles this — it uses a lockstep versioning strategy that bumps every package in the plugin together. For smaller plugins, just remember to bump all three package.json files before publishing.
