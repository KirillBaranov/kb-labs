...
| Mode | Best for | Guide |
|---|---|---|
| **Local dev** | Plugin development, testing | This page → Mode 1 |
| **Single-host cloud** | Teams, internal tools, self-hosted SaaS | [Self-Hosted Cloud →](/operations/self-hosted-cloud) |
| **Distributed** | Horizontal scaling, Kubernetes | This page → Mode 3 |

> **Deploying to a VPS?** The recommended path is the CI-driven deployment covered in [Self-Hosted Cloud](/operations/self-hosted-cloud). It uses `kb-deploy apply` and GitHub Actions — no manual SSH for routine updates.
...
**The recommended path for single-host production is the CI-driven deployment described in [Self-Hosted Cloud →](/operations/self-hosted-cloud).** It uses `kb-deploy apply` to install and manage services via GitHub Actions — no manual build steps, no hand-written systemd units.

In short:
1. Add SSH and API secrets to your GitHub environment.
2. Run the **Deploy Platform (apply)** workflow — it builds the monorepo, publishes packages to a private Verdaccio on your VPS, and starts all services through `kb-dev`.
3. Run **provision-tenant** for each tenant subdomain — nginx config + Let's Encrypt SSL, fully automated.

See [Self-Hosted Cloud](/operations/self-hosted-cloud) for the full step-by-step guide.
...
- `plugins/rest-api/Dockerfile`
- `plugins/workflow/daemon/Dockerfile` (check for one)
- `plugins/marketplace/Dockerfile`
- `plugins/gateway/app/Dockerfile`
