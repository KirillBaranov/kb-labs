...
This page documents the workflow spec schema in full. Every field, every constraint, every default value is taken from [plugins/workflow/contracts/src/schemas.ts](plugins/workflow/contracts/src/schemas.ts) — the Zod schemas are the single source of truth.
...
interface WorkflowSpec {
  ...
  outputs?: Record<string, WorkflowOutputField>;
...
Optional workflow-level outputs exported when this workflow is invoked by another workflow. Each output has a source expression evaluated from completed step outputs:

```yaml
outputs:
  report:
    source: '${{ steps.verify.outputs.report }}'
    description: Human-readable verification report
```

The parent receives declared values in `steps.<child-step>.outputs.outputs`, alongside the child `runId`, terminal `status`, and `artifacts`. When no outputs are declared, the envelope falls back to successful addressable step outputs for diagnostics.
...
- **`'workflow:workspace:<workflow-id>'`** — invokes a discoverable workspace workflow as a wait-only child step. The engine persists parent linkage and workflow ancestry, rejects indirect cycles, and resumes the parked parent after a terminal child result.
...
type StepState =  RunState | 'waiting_approval' | 'waiting_child';
```

`'dlq'` (dead letter queue) is run-only — a run exhausts its retries and gets parked for manual inspection. `'interrupted'` is job-only — the job was mid-execution when the engine restarted. `'waiting_approval'` is step-only — the step is paused on `builtin:approval`; `'waiting_child'` means it is parked while a persisted child workflow runs.
