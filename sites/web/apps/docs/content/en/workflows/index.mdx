...
Source of truth: [plugins/workflow/contracts/src/schemas.ts](plugins/workflow/contracts/src/schemas.ts).
...
A workflow can invoke another workspace workflow as a step:
...
Invocation is wait-only: the parent step moves to `waiting_child`, the child runs independently, then the parent resumes with `{ runId, status, outputs, artifacts }`. The engine persists invocation links and reconciles them after restart, propagates cancellation to descendants, and rejects cycles from the complete ancestry chain. The child uses the root run's workspace binding, making this suitable for review-and-fix workflows that must inspect the parent change.

For production pipelines, `.kb/workflows/quality-assurance.yaml` is a reusable boundary for independent review, architecture review, adversarial QA, functional verification, and static gates. `task-to-pr` invokes it before delivery.
...
Custom step types come from plugins: `plugin:<plugin-id>:<handler-id>` resolves to a workflow handler declared in the plugin's manifest. `workflow:workspace:<workflow-id>` invokes a discoverable workspace workflow as a wait-only child step.
...
A workflow run goes through well-defined states defined in [`workflow-constants`](plugins/workflow/constants/src/index.ts):
...
A tiny embedded expression language runs inside `if` conditions and `${{ … }}` interpolation in string fields. Source: [expressions.ts](plugins/workflow/contracts/src/expressions.ts).
