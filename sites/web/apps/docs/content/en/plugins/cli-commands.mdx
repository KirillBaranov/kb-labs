...
3. Clearing the registry cache (`pnpm kb marketplace plugins refresh`),
...
If you're not sure why a command isn't visible, run the trace tool:

```bash
kb diag --command "your-group your-command"
```

It walks the full discovery pipeline (registry → manifest → marketplace lock → filesystem) and tells you exactly which stage failed and what to fix.
...
Turning on `LOG_LEVEL=debug` shows the resolved handler paths at plugin load time. For a structured diagnosis across all pipeline stages, use `kb diag --command <your-command>` — it pinpoints whether the issue is the manifest schema, a missing `dist/`, a disabled marketplace entry, or a missing dependency.
