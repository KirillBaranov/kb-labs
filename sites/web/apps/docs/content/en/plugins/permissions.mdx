...
interface PermissionSpec {
  ...
  shell?: { allow?: string[]; maxConcurrent?: number };
  quotas?: {
    timeoutMs?: number;
    memoryMb?: number;
    cpuMs?: number;
...
const pluginPermissions = combinePermissions()
  ...
  .withShell({ allow: ['git status', 'git diff'], maxConcurrent: 4 })
...
| `.withShell(shell)` | Add `{ allow: [...], maxConcurrent? }` — command whitelist plus an optional cap on concurrent shell processes for this plugin. |
...
- **Scalars** (quotas, boolean platform flags, `shell.maxConcurrent`) — the later call wins.
