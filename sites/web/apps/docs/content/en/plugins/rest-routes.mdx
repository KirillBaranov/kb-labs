...
For one-way, long-lived responses, use the sibling `sse` manifest section with `defineEventStream` instead of modelling the connection as a normal `rest.routes` handler.
...
Use SSE when the server needs to push progress, logs or state changes to a client but the client does not need to send messages over the same connection. It is distinct from a normal REST route: the connection is long-lived, is automatically correlated in logs, and has a host-owned sender.

### Manifest declaration

`sse` is a top-level transport section, parallel to `ws`. It keeps long-lived connection policy out of ordinary request-response `rest.routes` declarations.
...
sse: {
  ...
  streams: [{
    path: '/events',
    handler: './sse/events.js#default',
    description: 'Agent progress events',
    security: ['token'],
  }],
}
```

`path` is relative to `sse.basePath`; when `sse.basePath` is omitted, the REST host uses the plugin's REST base path. `timeoutMs`, `keepAliveMs`, `security`, and `permissions` can be overridden for an individual stream. A stream cannot reuse the path of a `GET` route in the same plugin.

### Handler lifecycle

```ts
import { defineEventStream } from '@kb-labs/sdk';

export default defineEventStream({
  path: '/events',
  handler: {
    onConnect(_ctx, stream) {
      stream.send('connected', { ok: true });
      const unsubscribe = eventBus.subscribe('agent.progress', (event) => {
        stream.send('agent.progress', event);
      });
      stream.onCleanup(unsubscribe);
    },
    onDisconnect() {},
    onError(_ctx, error) {
      // Optional plugin-specific error handling.
      console.error(error);
    },
  },
});
...
The REST host invokes `onConnect`, then keeps the connection alive. `onError` receives transport errors, and `onDisconnect` runs during cleanup. The host logs connection open/close, duration, bytes and event counts; plugins should not write to `reply.raw` or manage SSE framing themselves.
