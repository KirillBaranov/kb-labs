...
This page is the canonical reference for schema `kb.plugin/3`. It mirrors the TypeScript types in [core/plugin-contracts/src/manifest.ts](core/plugin-contracts/src/manifest.ts) — if the code disagrees with the docs, the code wins.
...
Server-sent events are a one-way realtime transport: the client opens an HTTP connection and the plugin sends named events until it closes. `sse` intentionally mirrors the transport-level shape of `ws`, rather than living inside `rest.routes`.
...
`basePath` defaults to the plugin REST base path. Stream paths are relative to it and must not overlap a `GET` route of the same plugin. A declaration inherits `timeoutMs` and `keepAliveMs` from `sse.defaults`; values on the stream win. The handler uses `defineEventStream` and runs in-process because its sender owns the live HTTP response.

```ts
sse: {
  basePath: '/v1/plugins/agents',
  defaults: { keepAliveMs: 30_000 },
  streams: [{
    path: '/events',
    description: 'Agent progress updates',
    handler: './sse/agent-events.js#default',
    security: ['token'],
  }],
}
```

See [Plugins → REST Routes](/plugins/rest-routes#server-sent-events-sse) for the handler lifecycle.
...
The loader in [core/plugin-contracts/src/manifest-loader.ts](core/plugin-contracts/src/manifest-loader.ts) enforces:
...
The most exercised reference manifest in the monorepo is [plugins/commit/entry/src/manifest.ts](plugins/commit/entry/src/manifest.ts). It demonstrates:
