...
The split isn't enforced — the runtime only cares about the manifest and the handler paths it points at. But every first-party plugin follows this layout, so copy it and save yourself the thinking. [commit plugin](plugins/commit/entry/) is the canonical reference.
...
The lock file ([schema `kb.marketplace/2`](core/discovery/src/types.ts)) records, for each installed entity:
...
When a handler fires, the [sandbox runner](core/plugin-runtime/src/sandbox/runner.ts) picks one of two modes:
...
- [ ] Copy the structure from [commit plugin](plugins/commit/entry/).
...
- [ ] Clear cache: `pnpm kb marketplace plugins refresh`.
