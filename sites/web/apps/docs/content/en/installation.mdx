...
```bash
curl -fsSL https://github.com/KirillBaranov/kb-labs/releases/download/v1.2.3/checksums.txt \
...
For environments where `curl | sh` is restricted, download the binary directly from [GitHub releases](https://github.com/KirillBaranov/kb-labs/releases/latest):
...
- Creates `.kb/kb.config.jsonc` with default adapter stubs
- Runs `pnpm install`
