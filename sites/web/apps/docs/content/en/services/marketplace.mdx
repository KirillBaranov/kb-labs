...
It's a small Fastify service. Source: [plugins/marketplace/daemon/](plugins/marketplace/daemon/).
...
Source: [plugins/marketplace/daemon/src/manifest.ts](plugins/marketplace/daemon/src/manifest.ts).
...
From [`bootstrap.ts`](plugins/marketplace/daemon/src/bootstrap.ts):

1. Call `runService()` with the marketplace app ID, assembly hook, and service setup.
2. `runService()` resolves roots, loads layered `.env`, and initializes the platform singleton.
3. Construct `MarketplaceService` with an `NpmPackageSource`.
4. Create a Fastify server via `@kb-labs/marketplace-api`.
5. Listen on `port` (default 5070).
6. Return teardown from setup; `runService()` owns graceful shutdown.
...
The lock file is **the single source of truth** for the discovery layer. Every CLI startup and every service bootstrap reads this file via `DiscoveryManager.discover()` ([core-discovery](core/discovery/src/discovery-manager.ts)) and loads the manifests it points at.
