...
1. Calls `runService()` from `@kb-labs/shared-daemon`, which resolves platform/project roots and loads layered `.env`.
2. `runService()` initializes the global platform singleton — adapters, logger, cache, storage, everything declared in `kb.config.json`.
