...
Source lives in [plugins/workflow/daemon/](plugins/workflow/daemon/). The engine itself is in a sibling package, `@kb-labs/workflow-engine`.
...
Source: [packages/workflow-daemon/src/manifest.ts](plugins/workflow/daemon/src/manifest.ts).
...
From [`bootstrap.ts`](plugins/workflow/daemon/src/bootstrap.ts):

1. `runService()` resolves roots, loads layered `.env`, and initializes the platform singleton (adapters, logger, cache, storage).
...
7. Return teardown; `runService()` registers shutdown handling for `SIGTERM` / `SIGINT`.
...
Routes are registered in [`server.ts`](plugins/workflow/daemon/src/server.ts). Five sub-APIs, plus health and OpenAPI:
