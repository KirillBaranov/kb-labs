...
When a pack defines dependency boundaries, `kb-devkit check` validates static TypeScript and JavaScript `from` imports against the package's allow/forbid rules. Imports shown only in comments, string examples, or template literals are ignored, so documentation and diagnostic snippets do not produce false violations.
...
1. `kb-devkit` collects all files matching the `inputs:` glob patterns for the `(package × task)` pair and computes a SHA256 hash of their contents and paths. A package manifest is always included when present; task command, input/output declarations, dependency graph, cache setting, toolchain versions, and the executing binary are also fingerprinted.
...
**Each task has its own independent cache.** The cache key combines task name, package, task semantics, toolchain fingerprint, and input hash — running `build` does not populate the `lint` cache. If you run `build` then `lint`, lint executes fresh. If you run `build` again, build is served from cache. Tasks are only linked through `deps:` — if `lint` declares `deps: ["build"]`, build runs first (from cache), then lint runs fresh. To make generated dependency output affect a consumer's cache key, list that output (for example `^dist/**`) in the consumer's `inputs:`.
...
- [Source on GitHub](https://github.com/kb-labs-team/kb-labs/tree/main/tools/kb-devkit) — Go source, issues, releases.
