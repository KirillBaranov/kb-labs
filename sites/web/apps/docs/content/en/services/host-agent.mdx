...
Source lives in [plugins/host-agent/](plugins/host-agent/). There's no `ServiceManifest` for it — the host agent is not managed by `kb-dev` because it's meant to run on end-user machines, not inside the platform's service stack.
