...
- **`shell`** — shell commands. Whitelist of specific commands, plus an optional `maxConcurrent` cap on how many shell processes the process broker will run for this plugin at once (unset means the broker's own default). perPackage-style checks that batch shell calls should keep their internal batch size at or below this value — batching above it just queues extra work behind the broker instead of running it, and can blow the check's own timeout.
