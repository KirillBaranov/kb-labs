...
```
  Developer
     ...
     └──── browser  ────────────▶  Studio (port 3000)
          ...
          ┌──────────────┬──────────────┼──────────────┬──────────────┐
          ▼              ▼              ▼              ▼              ▼
   REST API (:5050)  Workflow (:7778) Marketplace (:5070) MCP (:7779)  …
          │              │                                  │
          └──────────────┼──────────────────────────────────┘
                           ▼
                   Plugin Runtime
...
### MCP daemon (port 7779)

Exposes the same plugin commands the CLI runs as [Model Context Protocol](https://modelcontextprotocol.io) tools, so external agents can drive the platform. The tool catalog is auto-generated from plugin manifests — no hardcoding — and filtered per identity by the policy decision point: an authenticated caller sees only the tools it may use, an anonymous one sees an empty catalog. Tool calls route through the same execution pipeline as the CLI.
