...
- **Workflow call** — `uses: "workflow:workspace:<workflow-id>"` to compose workflows.
...
## Composing workflows safely

A child workflow is a durable invocation, not a shell-out. The parent step enters `waiting_child`, releases its worker slot, and resumes only after the child reaches a terminal state. The link is stored with parent run/job/step IDs, so startup reconciliation completes or fails parked parents after a daemon restart.

The engine records the full workflow ancestry and rejects cycles such as `release → quality → release`; it also enforces `maxWorkflowDepth`. Cancelling a parent cascades to active descendants. A child borrows its root run's workspace binding, so a reusable quality workflow reviews the same uncommitted change rather than a fresh checkout.
