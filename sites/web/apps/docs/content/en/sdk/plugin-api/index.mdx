...
For realtime endpoints, the same SDK exposes `defineWebSocket` for bidirectional channels and `defineEventStream` for server-to-client SSE. Both are declared in the plugin manifest; see [REST Routes → Server-Sent Events](/plugins/rest-routes#server-sent-events-sse).
...
The `marketplace plugins refresh` step is mandatory — without it, the CLI uses a cached registry and won't see your new plugin.
