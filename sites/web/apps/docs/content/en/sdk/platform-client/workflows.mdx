...
Cancellation is cooperative — the engine marks the run as `'cancelled'` and stops scheduling new jobs. In-flight jobs finish or abort based on their handler's response to `ctx.signal`. If this run invoked child workflows, cancellation cascades through that persisted invocation tree; a daemon restart does not lose the links.

## Composed workflow runs

Runs started by `workflow:workspace:<id>` steps have `trigger.parentRunId`, `parentJobId`, and `parentStepId`. Their `metadata.workflowAncestors` records the complete workflow lineage, which lets clients link a child back to its parent and lets the engine reject indirect cycles. A successful child exports a stable parent-step envelope:

```ts
{ runId: 'child-run-id', status: 'success', outputs: {}, artifacts: [] }
```

Use `get` on either run ID for full detail; Studio and `kb workflow runs view` show the parent and lineage directly.
