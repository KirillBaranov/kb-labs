...
Role names are validated against the platform's canonical capability list — a typo'd role fails before any package installs. Most common roles (`llm`, `storage`, `logger`, `analytics`, ...) already have a sane default wired from the manifest catalog; `cache` deliberately has none, since only one package in the ecosystem implements it today and defaulting to it would fail at runtime wherever Redis isn't running. If a plugin declares a capability as required and nothing backs it, the install still succeeds but prints a visible warning — check the output when adding a new plugin to a CI job for the first time.
