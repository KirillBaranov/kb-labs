...
Source lives in [plugins/gateway/](plugins/gateway/).
...
Source: [plugins/gateway/app/src/manifest.ts](plugins/gateway/app/src/manifest.ts).

## Configuration

The gateway reads its config from the `gateway` section of `.kb/kb.config.json`. The schema is enforced by [`GatewayConfigSchema`](core/gateway/contracts/src/config.ts):
...
```bash
cd plugins/gateway/apps/gateway-app
node ./dist/index.js
```
