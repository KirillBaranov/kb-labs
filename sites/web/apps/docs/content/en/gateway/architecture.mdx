...
Source: [plugins/gateway/app/src/](plugins/gateway/app/src/).
...
From [`bootstrap.ts`](plugins/gateway/app/src/bootstrap.ts):

1. **Launch service.** `runService({ appId: 'gateway', startDir, platform: { assemblyHook }, setup })` resolves roots, loads layered `.env`, and initializes the platform singleton (logger, cache, storage, optionally a SQL adapter).
...
[`HostRegistry`](plugins/gateway/app/src/hosts/registry.ts) is the data structure that tracks which hosts are currently connected. Two-layer design:
...
[`dispatcher.ts`](plugins/gateway/app/src/hosts/dispatcher.ts) is what the `/internal/dispatch` endpoint calls when it needs to execute something on a host agent. Given a `namespaceId` (or a specific `hostId`), the dispatcher:
...
- **`/hosts/connect`** — for host agents connecting inbound. Authenticated via Bearer JWT. Managed by [`ws-handler.ts`](plugins/gateway/app/src/hosts/ws-handler.ts).
...
[`/internal/dispatch`](plugins/gateway/app/src/execute/routes.ts) is a gateway-internal endpoint used by the container execution backend. When `platform.execution.mode === 'container'`, the REST API (or workflow daemon) forwards execution requests through this endpoint to a container environment.
