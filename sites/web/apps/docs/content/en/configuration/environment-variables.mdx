...
`runService()` loads `.env` from the project root and, when it is distinct, the platform root before reading adapters or product config. Values already in the environment are not overwritten — an existing `OPENAI_API_KEY` in the parent shell wins over the one in `.env`.
...
3. `.env` file at the platform root, when it is distinct from the project root.
4. Defaults hardcoded in the source.
