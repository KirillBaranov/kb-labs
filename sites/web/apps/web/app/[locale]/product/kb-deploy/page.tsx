...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'kbDeploy' });
...
export default async function KbDeployPage({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'kbDeploy' });
  const deployCommands = t.raw('commands') as CommandItem[];
  const deployFeatures = t.raw('features') as FeatureItem[];
        ...
        <Section className="bg-surface/50">
          <Container>
            ...
            <AnimateOnScroll delay={80}>
              ...
              <p className="mt-3 text-center font-mono text-[0.65rem] text-muted/45 dark:text-muted/50">
                {t('commandsFootnote')}
              </p>
            </AnimateOnScroll>
          </Container>
