    ...
    <div className="flex flex-col gap-6 sm:flex-row sm:gap-10 sm:items-start">
      ...
      <nav className="flex flex-row flex-wrap gap-x-4 gap-y-4 sm:w-40 sm:shrink-0 sm:flex-col sm:gap-6">
        <div className="flex flex-col gap-0.5">
          <p className="mb-2 px-3 text-[0.6rem] font-bold uppercase tracking-[0.1em] text-muted/55 dark:text-muted/50">
          ...
          {TYPE_OPTIONS.map((type) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-xs tabular-nums ${isActive ? 'text-accent/60' : 'text-muted/45 dark:text-muted/50'}`}>
                  {typeCounts[type]}
                </span>
              </button>
            );
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mb-6 flex items-center gap-3">
          <Input
            ...
            className="flex-1"
          />
          <p className="shrink-0 font-mono text-[0.75rem] text-muted/55 dark:text-muted/50">
            ...
            {filtered.map((item) => (
              ...
              >
                <div className="flex h-full flex-col gap-3 rounded-2xl border border-line bg-surface p-5 transition-all duration-200 group-hover:border-accent/30 group-hover:bg-surface/80">
                  ...
                  {/* Footer */}
                  <p className="mt-auto font-mono text-[0.7rem] text-muted/50 dark:text-muted/50">by {item.author}</p>
                </div>
              </a>
            ))}
          </div>
