        ...
        <Container className="py-8 pb-24">
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start lg:gap-12">
            ...
            <div className="min-w-0 flex-1 flex flex-col gap-8">
              ...
              {/* Install command */}
              <div>
                <p className="mb-2 text-[0.6rem] font-bold uppercase tracking-[0.1em] text-muted/40">
                  ...
                  <div className="flex flex-col divide-y divide-line rounded-xl border border-line">
                    {item.commands.map((cmd) => (
                      <div key={cmd} className="flex items-center gap-4 px-4 py-3">
                        ...
                        <span className="ml-auto font-mono text-xs text-muted/35">--help</span>
                      </div>
                    ))}
                  </div>
                </section>
                ...
                <section>
                  <details className="rounded-xl border border-line">
                    ...
                    <div className="grid gap-4 border-t border-line p-4 sm:grid-cols-3">
                      {item.permissions && item.permissions.length > 0 && (
                        <div className="flex flex-col gap-1.5">
                          <p className="text-[0.6rem] font-bold uppercase tracking-[0.1em] text-muted/40">{t('marketplace.detail.permissionsLabel')}</p>
                      ...
                      {item.allowedHosts && item.allowedHosts.length > 0 && (
                        <div className="flex flex-col gap-1.5">
                          <p className="text-[0.6rem] font-bold uppercase tracking-[0.1em] text-muted/40">{t('marketplace.detail.networkLabel')}</p>
                      ...
                      {item.envVars && item.envVars.length > 0 && (
                        <div className="flex flex-col gap-1.5">
                          <p className="text-[0.6rem] font-bold uppercase tracking-[0.1em] text-muted/40">{t('marketplace.detail.envLabel')}</p>
                          {item.envVars.map((v) => (
            ...
            <aside className="w-full shrink-0 lg:w-64 lg:sticky lg:top-24 lg:self-start">
              ...
              {item.tags.length > 0 && (
                <div className="mt-5 flex flex-col gap-2">
                  <p className="text-[0.6rem] font-bold uppercase tracking-[0.1em] text-muted/40">
