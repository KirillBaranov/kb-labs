...
All three live in your plugin's CLI package. The canonical example is [kb-labs-commit-plugin](plugins/kb-labs-commit-plugin/packages/commit-cli/src/).

## 1 — The manifest entry

From [kb-labs-commit-plugin's manifest](plugins/kb-labs-commit-plugin/packages/commit-cli/src/manifest.ts):
