...
For reference, [kb-labs-commit-plugin](plugins/kb-labs-commit-plugin/) follows this pattern exactly.
...
The marketplace data model has a `signature` field on `MarketplaceEntry` (see [core-discovery types](platform/kb-labs-core/packages/core-discovery/src/types.ts)):
