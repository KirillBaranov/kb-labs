...
Defined in [platform/kb-labs-shared/packages/shared-perm-presets/src/types.ts](platform/kb-labs-shared/packages/shared-perm-presets/src/types.ts):
...
Defined in [infra/kb-labs-plugin/packages/plugin-contracts/src/permissions.ts](infra/kb-labs-plugin/packages/plugin-contracts/src/permissions.ts):
...
The runtime `platform` subtree adds more fine-grained control for lifecycle APIs — see [the runtime type](infra/kb-labs-plugin/packages/plugin-contracts/src/permissions.ts) for `workflows`, `jobs`, `cron`, `environment`, `workspace`, `snapshot`, and `execution`. Each of those is either a boolean or an object with per-capability flags (e.g. `workflows: { run: true, list: true, workflowIds: ['my-wf-*'] }`).
...
All eight presets live in [platform/kb-labs-shared/packages/shared-perm-presets/src/presets/](platform/kb-labs-shared/packages/shared-perm-presets/src/presets/). They're exported from `@kb-labs/sdk` with a `Preset` suffix.
...
Even if you set `permissions: undefined`, the runtime applies `DEFAULT_PERMISSIONS` from [plugin-contracts/src/permissions.ts](infra/kb-labs-plugin/packages/plugin-contracts/src/permissions.ts):
