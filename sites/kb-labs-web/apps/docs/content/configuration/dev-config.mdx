...
The reference file in this repo is [.kb/dev.config.json](.kb/dev.config.json) — read it alongside this page for a real example.
...
If the service is a platform daemon, it should also have a [`ServiceManifest`](/services#service-manifests) (`kb.service/1`) in its source — that manifest is what lets `kb-dev` know how to install and update it when the service is published to npm. See [Services → Overview](/services) for the relationship between `ServiceManifest` and `dev.config.json`.
...
- [Services → Overview](/services) — how `ServiceManifest` and `dev.config.json` relate.
