...
These tell the runtime where to find `.kb/kb.config.json` and `node_modules/@kb-labs/*`. See [core-workspace README](platform/kb-labs-core/packages/core-workspace/README.md) for the full resolution algorithm.
...
Read at `useConfig()` time in [`core-runtime/src/adapters/config-adapter.ts`](platform/kb-labs-core/packages/core-runtime/src/adapters/config-adapter.ts). Unknown profile IDs fall back to `'default'`.
...
The CLI deliberately defaults to silent — no logs appear unless you set `LOG_LEVEL`, `KB_LOG_LEVEL`, or pass `--debug`. This is so `pnpm kb <command>` output is clean by default and users can opt into verbosity. See [`cli-bin/src/bin.ts`](platform/kb-labs-cli/packages/cli-bin/src/bin.ts).
...
Used by the REST API rate-limit middleware and metrics collector ([`rest-api/src/middleware/rate-limit.ts`](platform/kb-labs-rest-api/apps/rest-api/src/middleware/rate-limit.ts)). Requests carrying an explicit `X-Tenant-ID` header override the env var; requests without it fall back to this value.
...
See [`gateway-app/src/bootstrap.ts`](infra/kb-labs-gateway/apps/gateway-app/src/bootstrap.ts) and [Gateway → Authentication](/gateway/authentication).
...
From [`marketplace-cli/src/http.ts`](platform/kb-labs-marketplace/packages/marketplace-cli/src/http.ts). `KB_MARKETPLACE_URL` takes precedence when present.
...
All four `KB_*` flags read by [`core-sandbox/src/runner/initialization/observability-setup.ts`](platform/kb-labs-core/packages/core-sandbox/src/runner/initialization/observability-setup.ts).
