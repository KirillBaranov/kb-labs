...
Every hook in this page is exported from `@kb-labs/sdk`. The actual implementations live in [`@kb-labs/shared-command-kit`](platform/kb-labs-shared/packages/shared-command-kit/src/helpers/) and read from `platform` in `@kb-labs/core-runtime`.
...
As documented in [use-platform.ts](platform/kb-labs-shared/packages/shared-command-kit/src/helpers/use-platform.ts), the platform exposes:
