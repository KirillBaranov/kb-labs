...
Local dev uses `.kb/dev.config.json` for service management and `.kb/kb.config.json` for platform config. Both files live in the workspace root. See [Configuration → dev.config.json](/configuration/dev-config) and [Configuration → kb.config.json](/configuration/kb-config) for the full schemas.
