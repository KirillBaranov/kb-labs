...
A workflow starts when a trigger fires. The `on` field of a `WorkflowSpec` declares which triggers a workflow responds to. At least one trigger must be set — `refine` on [`WorkflowTriggerSchema`](platform/kb-labs-workflow/packages/workflow-contracts/src/schemas.ts) enforces this.
