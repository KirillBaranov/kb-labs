...
Runs a shell command via [execa](platform/kb-labs-workflow/packages/workflow-builtins/src/shell.ts). Parameters:
...
Via [`parseWorkflowUses()`](platform/kb-labs-workflow/packages/workflow-contracts/src/expressions.ts), this resolves to `{ type: 'workflow', workflowId: 'security-audit', mode: 'wait', inheritEnv: true }`. The child run's trigger records `parentRunId`, `parentJobId`, `parentStepId`, and `invokedByWorkflowId` so you can trace the call stack.
...
A `StepRun` transitions through the following states (from [workflow-constants](platform/kb-labs-workflow/packages/workflow-constants/src/index.ts)):
