...
Это редкий, разовый случай — не общий механизм обхода checks. Общий
`--skip-checks` (и любой прямой `kb release run` в обход воркфлоу)
остаётся полностью запрещён (см. критическое правило №1 выше) и
физически блокируется защитным классификатором среды выполнения агента.
